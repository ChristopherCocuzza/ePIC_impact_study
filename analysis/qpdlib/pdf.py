...
def plot_xf_main(wdir,Q2,mode=0,SETS={}):
    ...
    for flav in data['XF']:
        ...
        y0,y1 = 16, 2.5
        up0 = np.percentile(data['XF'][flav],     y0, axis=0)
        do0 = np.percentile(data['XF'][flav], 100-y0, axis=0)
        up1 = np.percentile(data['XF'][flav],     y1, axis=0)
        do1 = np.percentile(data['XF'][flav], 100-y1, axis=0)
        ...
        if mode==2:
            ...
            thy  = ax.fill_between(X,do0,up0,color='red',alpha=0.7,zorder=5,where=np.array(where))
            axL      .fill_between(X,do0,up0,color='red',alpha=0.7,zorder=5,where=np.array(where))
            #thy  = ax.fill_between(X,do1,up1,color='red',alpha=0.7,zorder=5,where=np.array(where))
            #axL      .fill_between(X,do1,up1,color='red',alpha=0.7,zorder=5,where=np.array(where))
...
def plot_xf_du(wdir,Q2,mode=0,SETS={}):
    ...
    for flav in data['XF']:
        ...
        if mode==2:
            ...
            thy  = ax.fill_between(X,do0,up0,color='red',alpha=1.0,zorder=5)
            thy  = ax.fill_between(X,do1,up1,color='red',alpha=1.0,zorder=5)
...
#--pdf moments
def gen_moments(wdir, Q2 = 10, mom = 1, xmin = 1e-5, xmax = 0.95):
    ## get truncated second moment integrated from a range of x_min to 1
    load_config('%s/input.py' % wdir)
    istep = core.get_istep()

    flavors = ['quark','g','up-dp']
    ...
    mean = np.mean(moments['up-dp'],axis=0)
    std  = np.std (moments['up-dp'],axis=0)
...
def gen_moments_full(wdir, Q2 = 10, mom = 1):
    ## get truncated second moment integrated from a range of x_min to 1
    load_config('%s/input.py' % wdir)
    istep = core.get_istep()
    if Q2 == None: Q2 = conf['Q20']
    ...
    print('\ngenerating full moment %d for pdf-proton from %s at Q2 = %3.2f' % (mom, wdir, Q2))
    ...
    for i in range(n_replicas): ## using 'scipy.integrate.cumtrapz' takes about 9.984 seconds for 100 x points, 4 flavors and 516 replicas
        ...
        for flavor in flavors:
            if flavor not in moments:
                moments[flavor] = np.zeros(n_replicas)
            if flavor == 'quark':
                func = pdf.storage[Q2]['u'][N] + pdf.storage[Q2]['ub'][N] + pdf.storage[Q2]['d'][N] + pdf.storage[Q2]['db'][N] + \
                                           pdf.storage[Q2]['s'][N] + pdf.storage[Q2]['sb'][N] + pdf.storage[Q2]['c'][N] + pdf.storage[Q2]['cb'][N]
            elif flavor == 'db-ub':
                func = pdf.storage[Q2]['db'][N] - pdf.storage[Q2]['ub'][N]
            elif flavor == 'up-dp':
                func = pdf.storage[Q2]['u'][N] + pdf.storage[Q2]['ub'][N] - pdf.storage[Q2]['d'][N] - pdf.storage[Q2]['db'][N]
            else:
                func = pdf.storage[Q2][flavor][N]

            moments[flavor][i] = np.real(func)
    print()
    ...
    checkdir('%s/data' % wdir)
    save({'X': xs, 'Q2': Q2, 'moments': moments}, '%s/data/pdf-moment-full-%d-Q2=%3.5f.dat' % (wdir,mom,Q2))
