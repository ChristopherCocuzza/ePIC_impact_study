...
def get_replicas(wdir,mod_conf=None,inspected=True):
    """
    load the msr files
    """
    if inspected: replicas=sorted(os.listdir('%s/msr-inspected'%wdir))
    else:         replicas=sorted(os.listdir('%s/msr'%wdir))
    if inspected: replicas=[load('%s/msr-inspected/%s'%(wdir,_)) for _ in replicas]
    else:         replicas=[load('%s/msr/%s'%(wdir,_)) for _ in replicas]
    ...
    dists = conf['steps'][istep]['active distributions'] + (conf['steps'][istep]['passive distributions'])
    for dist in dists:
        flag = True
        for par in conf['params'][dist]:
...
def get_ci(data,CL=95):

    ci_min = np.percentile(data,100-CL,axis=0) 
    ci_max = np.percentile(data,CL    ,axis=0) 
