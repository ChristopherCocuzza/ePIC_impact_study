...
#--from tools
from tools.tools     import load,save,checkdir,lprint, mahal_dist
...
def get_msr_inspected(wdir,limit=3):
    ...
    #--i.e. ('pdf','dv1 a','<',0) filters out any replica with pdf dv1 a less than 0
    ...
    if limit==None: limit = 1000000000
...
def get_msr_inspected_mahal(wdir,quantile=1-1e-10):

    print('\nget msr inspected [mahal distance] (filtered msr files) using %s\n'%wdir)
    load_config('%s/input.py'%wdir)
    ...
    #--array for Mahal distance (size n_datasets, n_replicas)
    nreplicas = len(replicas)
    nexp = 0
    data=load('%s/msr/%s'%(wdir,replicas[0]))['chi2']
    for reaction in data: 
        for idx in  data[reaction]:
            nexp +=1
    ...
    for i in range(len(replicas)):
        ...
        for reaction in data: 
            for idx in  data[reaction]:
                chi2_exp = data[reaction][idx]['chi2']
                chi2+=data[reaction][idx]['chi2']
                npts+=data[reaction][idx]['npts']
                MAHAL.append(chi2_exp)
        CHI2.append(chi2)

    print(nexp)
    MAHAL = np.array(MAHAL).reshape((len(replicas),nexp)).T
    mahal,dist = mahal_dist(MAHAL,quantile=quantile)
    ...
    #py.tight_layout()
    checkdir('%s/gallery'%wdir)
    py.savefig('%s/gallery/mahalanobis_dist.png'%(wdir))
    py.close()

    for i in range(len(replicas)):
        if mahal[i]==False: continue
        X.append(CHI2[i]/npts-1)
        cmd=['cp']
        cmd.append('%s/msr/%s'%(wdir,replicas[i]))
        cmd.append('%s/msr-inspected/%s'%(wdir,replicas[i]))
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE)
...
def filter_values(resman,parman,par,order,filt):
    ...
    if dist=='ht4':
        ...
        tar = func[-1]
        stf = func[:2]
        value = DIST.get_ht(X,Q2,tar,stf)
