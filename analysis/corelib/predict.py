...
def get_predictions(wdir,force=False,ncores=1,mod_conf=None,name=''):
    ...
    #data1 = np.array(data['reactions']['idis'][1]['prediction-rep'])
    #data2 = np.array(data['reactions']['idis'][2]['prediction-rep'])
    #data3 = np.array(data['reactions']['idis'][3]['prediction-rep'])
    ...
    #sheet1 = pd.read_excel('BONuS12dp.xlsx').to_dict(orient='list')
    #sheet2 = pd.read_excel('BONuS12nd.xlsx').to_dict(orient='list')
    #sheet3 = pd.read_excel('BONuS12np.xlsx').to_dict(orient='list')

    ##for n in range(len(data1)):
    ##    sheet1['JAM replica %d'%n] = data1[n]
    ##    sheet2['JAM replica %d'%n] = data2[n]
    ##    sheet3['JAM replica %d'%n] = data3[n]


    #sheet1['JAM mean'] = mean1
    #sheet2['JAM mean'] = mean2
    #sheet3['JAM mean'] = mean3
    #sheet1['JAM std']  = std1
    #sheet2['JAM std']  = std2
    #sheet3['JAM std']  = std3


    #pd.DataFrame(sheet1).to_excel('BONuS12dp.xlsx',index=False)
    #pd.DataFrame(sheet2).to_excel('BONuS12nd.xlsx',index=False)
    #pd.DataFrame(sheet3).to_excel('BONuS12np.xlsx',index=False)


    #print('Updating BONuS data sets...')
    #--close resman
    resman.shutdown()
