...
def get_Q2bins(data,kind):
    ...
    if kind == 'CLAS12':
        query[8] = data.query('Q2 > 8.5') 
        query[7] = data.query('Q2 > 7.5 and Q2 <= 8.5') 
        query[6] = data.query('Q2 > 6.5 and Q2 <= 7.5') 
        query[5] = data.query('Q2 > 5.5 and Q2 <= 6.5') 
        query[4] = data.query('Q2 > 4.5 and Q2 <= 5.5') 
        query[3] = data.query('Q2 > 4.0 and Q2 <= 4.5') 
        query[2] = data.query('Q2 > 3.5 and Q2 <= 4.0') 
        query[1] = data.query('Q2 > 3.0 and Q2 <= 3.5') 
        query[0] = data.query('Q2 > 2.5 and Q2 <= 3.0') 
...
def get_plot_old(query,cluster_i=0):
...
def get_plot(query,cluster_i=0):
    ...
    for key in query:
        ...
        #--sort by ascending Q2
        zx = tuple(zip(Q2,X))
        zt = tuple(zip(Q2,theory))
        zv = tuple(zip(Q2,value))
        za = tuple(zip(Q2,alpha))
        zs = tuple(zip(Q2,std))
        if 'theta' in query[key]: ztheta = tuple(zip(Q2,theta))
        plot['X'][key]      = np.array([zx[i][1] for i in range(len(zx))])
        plot['theory'][key] = np.array([zt[i][1] for i in range(len(zt))])
        plot['value'][key]  = np.array([zv[i][1] for i in range(len(zv))])
        plot['alpha'][key]  = np.array([za[i][1] for i in range(len(za))])
        plot['std'][key]    = np.array([zs[i][1] for i in range(len(zs))])
        plot['Q2'][key]     = np.array(Q2)
        if 'theta' in query[key]: 
            plot['theta'][key] = np.array([ztheta[i][1] for i in range(len(ztheta))])
...
def get_details(exp):
    ...
    if exp=='JLab JLCEE96 p':  color, marker, ms = 'firebrick', 'o', 2
    if exp=='JLab JLCEE96 d':  color, marker, ms = 'darkgreen', 'o', 2 

    if exp=='CLAS12':  color, marker, ms = 'firebrick', 'o', 5
...
def plot_proton(wdir, data):
    ...
    if 10001 in data or 10003 in data:
        if 10001 in data: jlab_p = data[10001] ## JLab p 
        if 10003 in data: jlab_p = data[10003] ## JLab p 
        ...
        if 10001 in data: data[10001]['theta'] = theta(data[10001]['X'],data[10001]['Q2'],Mp)
        if 10003 in data: data[10003]['theta'] = theta(data[10003]['X'],data[10003]['Q2'],Mp)
        ...
        if 10003 in data:
            ax12.text(0.05,  0.70,   r'\boldmath$\sigma_{\rm red}$',         transform = ax12.transAxes, size = 60)
            ax12.text(0.15,  0.70,   r'$(\, \times\, 2^{\, i})$',            transform = ax12.transAxes, size = 40)
...
def plot_deuteron(wdir, data):
    ...
    if 10002 in data or 10004 in data:
        if 10002 in data: jlab_d = data[10002] ## JLab d 
        if 10004 in data: jlab_d = data[10004] ## JLab d 
        ...
        if 10002 in data: data[10002]['theta'] = theta(data[10002]['X'],data[10002]['Q2'],Mp)
        if 10004 in data: data[10004]['theta'] = theta(data[10004]['X'],data[10004]['Q2'],Mp)
...
def plot_JLab_E06_009(wdir, data):
    ...
    ax11.text(0.40, 0.20, r'$Q^2 = 2~{\rm GeV}^2$', transform = ax11.transAxes, size = 20)
    ax11.text(0.50, 0.55, r'$Q^2 = 3~{\rm GeV}^2$', transform = ax11.transAxes, size = 20)
    ax11.text(0.25, 0.90, r'$Q^2 = 4~{\rm GeV}^2$', transform = ax11.transAxes, size = 20)
    #ax11.text(0.05, 0.90, r'\boldmath$F_2^{^3\rm{He}}/F_2^{d}$', transform = ax11.transAxes, size = 30)
...
def plot_JLab_JLCEE96(wdir, data):
    ...
    ax11.set_ylim(0.15, 1.6)
    #ax11.set_yticks([])
    ax11.yaxis.set_minor_locator(MultipleLocator(0.10))
    ax11.yaxis.set_major_locator(MultipleLocator(0.20))

    ax12.set_ylim(0.15, 1.19)
    ax12.yaxis.set_minor_locator(MultipleLocator(0.10))
    ax12.yaxis.set_major_locator(MultipleLocator(0.20))


    ax11.text(0.03, 0.90, r'\textrm{\textbf{JLab JLCEE96 }}',transform = ax11.transAxes, size = 30)
    ax11.text(0.03, 0.80, r'\boldmath$F_2^p$', transform = ax11.transAxes, size = 40)
    ax12.text(0.03, 0.90, r'\boldmath$F_2^D$', transform = ax12.transAxes, size = 40)

    #ax12.text(0.60, 0.80, r'$E=5.011~{\rm GeV}$', transform = ax12.transAxes, size = 20)


    ax11.text(0.42, 0.08, r'$Q^2 = 1.7 ~ {\rm GeV}^2$', transform = ax11.transAxes, size = 20)
    ax11.text(0.69, 0.21, r'$Q^2 = 2.1$',               transform = ax11.transAxes, size = 20)
    ax11.text(0.17, 0.24, r'$Q^2 = 2.4$',               transform = ax11.transAxes, size = 20)
    ax11.text(0.29, 0.52, r'$Q^2 = 3.3$',               transform = ax11.transAxes, size = 20)

    ax12.text(0.41, 0.06, r'$Q^2 = 1.7 ~ {\rm GeV}^2$', transform = ax12.transAxes, size = 20)
    ax12.text(0.69, 0.21, r'$Q^2 = 2.1$',               transform = ax12.transAxes, size = 20)
    ax12.text(0.17, 0.22, r'$Q^2 = 2.4$',               transform = ax12.transAxes, size = 20)
    ax12.text(0.29, 0.52, r'$Q^2 = 3.3$',               transform = ax12.transAxes, size = 20)
